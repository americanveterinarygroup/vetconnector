...
logging_config.logger('./logs/uv_report_download.log')
...
uv_doctors = df[ df['location_name'].str.contains('UrgentVet') ]
uv_doctors.to_csv('./source_docs/uv_doctors.csv', index=False)
logging.info('uv doctor list retrieved')

chrome = selenium_pbi.set_chrome()
selenium_pbi.pbi_login(chrome)

pbi_uv_url = 'https://app.powerbi.com/groups/58a5001e-4b10-4c6e-a27f-05c157db4955/reports/b7d89b51-0586-45ce-a3d8-e202cc1911f0/ReportSection?experience=power-bi'
...
pbi_download = "C:/Users/RonaldEvans/Downloads/DVM Monthly Report - UrgentVet.pdf"
...
for idx in uv_doctors.index:
    try:
        ...
        sec = 5
        check_file = False
        save_pdf = f"C:/dvm_monthly_distribution/pdfs/uv_staging/{id}_DvmMonthlyReport.pdf"
